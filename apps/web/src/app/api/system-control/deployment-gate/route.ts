...
async function computeLogicRegression(symbol: string, limit: number) {
  ...
  for (const row of rows) {
    ...
    const hasMismatch = storedBuyVotes !== computedBuyVotes || storedSellVotes !== computedSellVotes || row.signal !== computedSignal;
  ...
  const ruleEngineHealth: RuleEngineHealthRow[] = Array.from(byRuleEngine.values())
    .map((item) => ({
      mode: item.mode,
  ...
  return {
    ...
    mismatches,
    pass: mismatches === 0,
    rule_engine_health: ruleEngineHealth,
  };
}
...
async function evaluateAndPersistGate(symbol: string, limit: number) {
  ...
  if (!logic.pass) {
    const topRuleEngine = logic.rule_engine_health?.find((item) => item.mismatches > 0);
    const ruleEngineContext = topRuleEngine
      ? ` [${topRuleEngine.mode}@${topRuleEngine.version}: ${topRuleEngine.mismatches}/${topRuleEngine.checked_cases}]`
      : '';
    reasons.push(`logic regression mismatch ${logic.mismatches}/${logic.checked_cases}${ruleEngineContext}`);
  }
  if (!golden.pass) {
...
export async function POST(request: Request) {
  try {
    ...
    if (!immutableAudit.valid) {
      return NextResponse.json(buildImmutableAuditLockPayload(immutableAudit), { status: 423 });
    }
